...
export const EXAMPLE_FLOWS = {
  bruteForce: {
    ...
    description: "Typical SSH brute force attack pattern from CSE-CIC-IDS2018",
    data: {
      dst_port: 0.0003,
      protocol: 0.3529,
      timestamp: 0.0432,
  ...
  benign: {
    ...
    description: "Legitimate web browsing traffic",
    data: {
      dst_port: 0.7967,
      protocol: 0.3529,
      timestamp: 0.9500,
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          threat_level: apiResponse.prediction === 1 ?
            (apiResponse.confidence > 0.9 ? 'Severe' :
             apiResponse.confidence > 0.7 ? 'High' :
             apiResponse.confidence > 0.5 ? 'Moderate' : 'Low') : 'Minimal',
          attack_type: apiResponse.prediction === 1 ? 'Network-based Brute Force' : null,
      ...
      return {
        ...
        error: {
          ...
          message: error.message || 'Failed to connect to Brute Force Detection API',
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          status: 'active',
          description: 'Random Forest model trained on CSE-CIC-IDS2018 dataset',
  ...
  async healthCheck() {
    try {
      ...
      return {
        ...
        error: {
          type: 'network',
          message: 'Brute Force API is not available',
          details: error
        }
      };
    }
