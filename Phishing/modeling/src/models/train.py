"""
ENTRENAMIENTO DE MODELOS para Phishing Email Detection
...
def train_models(X_train, X_test, y_train, y_test, config=None):
    ...
    if config is None:
        config = {
            'random_state': 42,
            'logistic_regression': {'max_iter': 1000, 'n_jobs': -1},
    ...
    models = {
        'Logistic Regression': LogisticRegression(
            ...
            **config.get('logistic_regression', {'max_iter': 1000, 'n_jobs': -1})
        ),
        'Random Forest': RandomForestClassifier(
            random_state=random_state,
            **config.get('random_forest', {'n_estimators': 100, 'max_depth': 10, 'n_jobs': -1})
        ),
        'SVM': SVC(
            random_state=random_state,
            **config.get('svm', {'kernel': 'rbf', 'probability': True})
        ),
...
def select_best_model(models_results, metric='f1_score'):
    ...
    Args:
        ...
        metric: Métrica para selección ('f1_score', 'accuracy', 'roc_auc', etc.)
...
def save_results(models_results, best_model_name, output_dir, X_train, y_test):
    ...
    # 6. Crear reporte comparativo en texto
    report_path = os.path.join(reports_dir, 'models_comparison_report.txt')
    with open(report_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("REPORTE DE COMPARACIÓN DE MODELOS - PHISHING DETECTION\n")
        f.write("="*70 + "\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Entrenamiento de modelos para Phishing Detection')
    parser.add_argument('--features_path', type=str, required=True, help='Path al CSV con features')
    parser.add_argument('--output_dir', type=str, required=True, help='Directorio de salida')
    parser.add_argument('--random_state', type=int, default=42, help='Random state para reproducibilidad')

    args = parser.parse_args()

    # Cargar features
    print(f"📂 Cargando features desde: {args.features_path}")
    ...
    # Como las features ya están divididas en train/test por el usuario,
    # no hacemos split aquí. Asumimos que se pasa el conjunto completo
    # y se hace split externamente o se pasan train/test por separado.

    print("⚠️ NOTA: Este script asume que los datos ya están divididos en train/test.")
    print("⚠️ Para uso completo, llama desde el notebook o pipeline que maneja el split.")

    print("\n🎯 Proceso completado. Usar desde notebook para ejecución completa.")
